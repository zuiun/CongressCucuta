...
Some examples are in the `Simulations` folder.
- `App.xaml` contains the entry point for the program.
This needs to be changed to `Views/CompilerWindow.xaml`.
- `ViewModels/CompilerViewModel.cs` stores compilable simulations.
Add your `ISimulation` to the `Simulations` field in the constructor.
- Build and run the project. The program will ask you to choose a file to save as `.sim`.
	...
	You can then run your `.sim` with the (theoretically) provided `.exe`.
